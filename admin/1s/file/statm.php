...
if($_POST['search']&&!empty($_POST['n_card']))	
{
	...
	$sql="SELECT id, number_card, f_name, s_name, t_name, balls, balance FROM member WHERE number_card=".$_POST['n_card']." ORDER BY ".$orderby;
}
else $sql="SELECT `id`, `number_card`, `f_name`, `s_name`, `t_name`, `balls`, `balance`, `parent` FROM `member` ORDER BY ".$orderby;
	$res=mysql_query($sql,$db);
	while($row=mysql_fetch_row($res))
	{
?>
<tr class="<? if($odd) {$odd=0; echo "even";}
 ...
 <td style="padding: 4px 12px;"><? echo $row[4].' '.substr($row[2],0,2).'.'.substr($row[3],0,2).'.';?></td>
 <td align="center" style="padding: 4px 12px;"><? 
