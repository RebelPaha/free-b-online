...
if( $_SESSION[ 'id_user' ] ){
    ...
    <div style="background: url(<? echo DIR_MAIN_IMAGES; ?>/pc_dline.png) repeat-x;">
        ...
        if( !isset( $_GET[ 'cabinet' ] ) )
            $_GET[ 'cabinet' ] = 0;
        $pc_menu = array( array( 'Профиль', '/page/' . NUMBER_PAGE_PCABINET . '/cabinet/0' ),
                          3 => array( 'Ваши Покупки', '/page/' . NUMBER_PAGE_PCABINET . '/cabinet/3' )
        , 2 => array( 'Сообщения', '/page/' . NUMBER_PAGE_PCABINET . '/cabinet/2' ),
//                          array( 'начисления <span style="text-transform:none;">f</span>B',
//                                 '/page/' . NUMBER_PAGE_PCABINET . '/cabinet/3'
//                          ),
//                          array( 'Рефералы', '/page/' . NUMBER_PAGE_PCABINET . '/cabinet/4' ),
//                          array( 'Деньги', '/page/' . NUMBER_PAGE_PCABINET . '/cabinet/5' )
        );
        ...
        foreach( $pc_menu as $key => $value ){
            if( $_GET[ 'cabinet' ] == $key ){
                ?>
                <div
                    style="background: url(<? echo DIR_MAIN_IMAGES; ?>/pc_choise_l.png) no-repeat; width:12px; height:42px; float:left;"></div>
                <div
                    style="float:left;background: url(<? echo DIR_MAIN_IMAGES; ?>/pc_choise_bg.png) repeat-x; height:42px; line-height:42px; padding-left:6px; padding-right:11px; font-weight:bold; text-transform:uppercase; color:#971d81;"><? echo $value[ 0 ]?></div>
                <div
                    style="float:left;background: url(<? echo DIR_MAIN_IMAGES; ?>/pc_choise_r.png) no-repeat; width:8px; height:42px; float:left;"></div>
            <?
            }
            else{
                ?>
        ...
        <div class="notify_box" style="height:312px;">
            <div class="notify_left">
                <form method="post" onsubmit="confirm('Вы правильно ввели расчетный счет (номер карты)?')">
                    ...
                    Укажите номер карточки (расчетный счет), куда Вам перечислить деньги... <p></p>
                    ...
                    <div align="center"><input type="submit" name="add_creditcard" value="Подтвердить"
                                               class="notify_btn"/></div>
                </form>
            </div>
        </div>
    <?
            ...
            <div class="notify_left">
                <form method="post" name="changedata" onsubmit="return checkcard()">
                    <table cellpadding="0" cellspacing="9" border="0">
                        <tr>
                            <td>e-mail:</td>
                            <td><input type="text" name="email" class="notify_input" maxlength="50"
                                       value="<? if( $_SESSION[ 'email' ] != '(пусто)' )
                        ...
                        <tr>
                            <td colspan="2" align="center" style="padding-top:15px;"><input type="submit"
                                                                                            name="send_check"
                                                                                            value="Изменить"
                                                                                            class="notify_btn"/><input
                                    type="button" onclick="javascript:history.back()" value="Отмена"
                                    style="margin-left:15px;" class="notify_btn"/></td>
                        </tr>
                    </table>
                </form>
            </div>
    ...
    elseif( $_GET[ 'edit' ] == 2 ){
            ...
            <form method="post">
                <table cellpadding="0" cellspacing="9" border="0">
                    <tr>
                        ...
                        <td width="400px;"><input type="text" id="cardCheck" name="card" class="notify_input"
                                                  maxlength="50" value="">

                            <div style="position:relative; " id="msgbox-card"></div>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2" align="center" style="padding-top:15px;"><input type="submit" id="submit"
                                                                                        name="cardCheck"
                                                                                        value="Сохранить"
                                                                                        class="notify_btn"
                                                                                        disabled="disabled"><input
                                type="button" onclick="goBack()" value="Отмена" style="margin-left:15px;"
                                class="notify_btn"/></td>
                    </tr>
                </table>
            </form>
        ...
        if( $_GET[ 'cabinet' ] == 1 || $_GET[ 'cabinet' ] == 3 )
            include( DIR_MAIN_FILE . '/history.php' );
        elseif( $_GET[ 'cabinet' ] == 2 )
            include( DIR_MAIN_FILE . '/mail.php' );
        elseif( $_GET[ 'cabinet' ] == 4 )
            include( DIR_MAIN_FILE . '/my_referal.php' );
        elseif( $_GET[ 'cabinet' ] == 5 )
            include( DIR_MAIN_FILE . '/my_money.php' );
        else{
            ...
            <div style="font-size:13px;font-weight: normal; margin-top:20px; margin-left:25px;">
                <a href="/page/<? echo $_GET[ 'page' ]; ?>/edit/1" class="pc_edit"><img
                        src="<? echo DIR_MAIN_IMAGES; ?>/pc_edit.png"
                        align="left" border="0" style="margin-right:5px; margin-top:-8px;"/>Изменить пароль или
                    e-mail</a><br>
            ...
            <div style="margin:20px; color:#971d81">
                <div style="float:left">
                    ...
                    <div style="float:left; line-height:34px;font-weight:bold; color:#666;">
                        <div
                            style="font-size:16px;"><? echo date( "d.m.Y", strtotime( $_SESSION[ 'datareg' ] ) ); ?></div>
                        <div style="font-size:16px;"><? echo $_SESSION[ 'user_name' ]; ?></div>
                        <div
                            style="font-size:13px; text-decoration: underline;font-weight: normal;"><? echo $_SESSION[ 'email' ]; ?></div>
                        <div
                            style="font-size:16px;"><? if( empty( $_SESSION[ 'birthday' ] ) || $_SESSION[ 'birthday' ] == '0000-00-00' )
                                echo '(пусто)';
                            else echo date( "d.m.Y", strtotime( $_SESSION[ 'birthday' ] ) ); ?></div>
                        <div style="font-size:16px;">
                            <img src="<? echo DIR_MAIN_IMAGES; ?>/pc_mob.png" align="left"
                                 style="margin-right:5px; margin-top:8px;"/>
                            <? if( $_SESSION[ 'phone' ] != '(пусто)' )
                                echo '(' . substr( $_SESSION[ 'phone' ], 0, 3 ) . ')&nbsp;' . substr( $_SESSION[ 'phone' ], 3, 3 ) . '-' . substr( $_SESSION[ 'phone' ], 6, 2 ) . '-' . substr( $_SESSION[ 'phone' ], 8 );
                            else echo '(пусто)';?></div>
                        <div style="font-size:16px;"><? echo $_SESSION[ 'sex' ]; ?></div>
                        <div style="font-size:16px;"><? echo $_SESSION[ 'city' ]; ?></div>
                        <div style="font-size:16px;"><? if( $_SESSION[ 'number_card' ] != NULL )
                                echo $_SESSION[ 'number_card' ];
                            else echo  $_SESSION[ 'inet_number' ]; ?></div>
...
<!--                    <div style="clear:left;"></div>-->
<!--                    <div style="font-size:18px; text-align:center; margin-top:20px;"><span-->
<!--                            style="font-size:36px;">--><?// echo $_SESSION[ 'balls' ]; ?><!-- fB</span></div>-->
<!--                    <div style="font-size:18px; text-align:center; margin-top:20px;"><span-->
<!--                            style="font-size:36px;">--><?// echo $_SESSION[ 'balance' ]; ?>
<!--                            грн.</span>--><?// if( $_SESSION[ 'balance' ] > 0 ){ ?>
<!--                            <form method="post">-->
<!---->
...
<!--                    <div-->
<!--                        style="font-size:18px; text-align:center; width:168px; height:34px; background:url(image/pc_econom.png)">--><?// echo $_SESSION[ 'saved_money' ];?>
<!--                        грн.-->
<!--                    </div>-->
<!--                </div>-->
...
<!--                <table cellpadding="10" cellspacing="1" border="0" bgcolor="#dcdfe1" width="700" align="center"-->
<!--                       style="margin-top:10px;">-->
<!--                    <tr bgcolor="#971d81" style="color:#fff; font-weight: bold;">-->
<!--                        <td align="center">Дата</td>-->
<!--                        <td align="center">Наименование</td>-->
<!--                        <td align="center">Баллов</td>-->
...
<!--                            <td align="center">--><?// if( $row[ 5 ] )
//                                    echo 'Активен';
//                                else echo 'Обрабатывается'; ?><!--</td>-->
<!--                        </tr>-->
<!--                    --><?//
//                    }
//                    ?>
